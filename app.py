...
with tab_dash:
    if df_base.empty:
        ...
        with k1:
            ...
            if st.button("🔍 Filtrar Asignadas", key="btn_asig", use_container_width=True): st.session_state['filtro_kpi'] = "Todas"
        with k2:
            ...
            if st.button("🔍 Filtrar Visitadas", key="btn_vis", use_container_width=True): st.session_state['filtro_kpi'] = "Visitadas"
        with k3:
            ...
            if st.button("🔍 Filtrar Pendientes", key="btn_novis", use_container_width=True): st.session_state['filtro_kpi'] = "No Visitadas"
        with k4:
            ...
            if st.button("🔍 Ver Todas", key="btn_cob", use_container_width=True): st.session_state['filtro_kpi'] = "Todas"
        ...
        with st.expander(f"{titulo_kpi} ({df_kpi_vista[col_agencia].nunique()} agencias)", expanded=False):
