...
with tab_dash:
    if df_base.empty:
        ...
        # 3. FILTRO POR FECHA CON OPCIÓN "VER TODO EL HISTÓRICO" POR DEFECTO
        st.sidebar.markdown("---")
        st.sidebar.subheader("📅 Consulta por Fecha")

        fechas_disponibles = sorted(df_base["Fecha_DT"].dropna().dt.date.unique())

        if len(fechas_disponibles) > 0:
            min_f = fechas_disponibles[0]
            max_f = fechas_disponibles[-1]
            
            modo_fecha = st.sidebar.radio("Periodo a consultar:", ["🗓️ Ver Todo el Histórico", "📆 Filtrar por Rango de Fechas"])
            
            if modo_fecha == "📆 Filtrar por Rango de Fechas":
        ...
        # Aplicar filtro por rango de fechas si fue activado
        if rango_fechas and isinstance(rango_fechas, tuple) and len(rango_fechas) == 2:
            f_inicio, f_fin = rango_fechas
            df_filtrado = df_filtrado[
                ((df_filtrado["Fecha_DT"].dt.date >= f_inicio) & (df_filtrado["Fecha_DT"].dt.date <= f_fin)) | 
        ...
        # BLOQUE 3: DASHBOARD TRANSACCIONAL Y OPERATIVO INFERIOR (100 FILAS)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        if "TipoTran" in df_filtrado.columns:
            ...
            st.subheader(f"🔍 Matriz Transaccional Completa ({total_trans} registros)")
