...
# Configuración de la zona horaria nativa para Quito, Ecuador (UTC-5)
...
# -----------------------------------------------------------------------------
# WIDGET DE CLIMA Y RELOJ DINÁMICO EN VIVO (JAVASCRIPT) EN SIDEBAR
# -----------------------------------------------------------------------------
reloj_html = """
<div style="
...
# -----------------------------------------------------------------------------
# FUNCIONES AUXILIARES DE GUARDADO SEGURO
# -----------------------------------------------------------------------------
...
def generar_reporte_word(df_data, col_agencia, col_region, col_coord, col_fecha, cols_eval):
    ...
    tot_asig = len(df_data) if not df_data.empty else 0
    ...
    p_body1.add_run(
        ...
        f"A la fecha de corte, se registra un universo total de {tot_asig} registros asignados en la matriz, de los cuales "
        f"{tot_vis} corresponden a visitas ejecutadas satisfactoriamente, representando un nivel de cobertura del {pct_cob:.1f}%. "
        f"Asimismo, se identifican {tot_novis} visitas pendientes."
    )

    table_kpi = doc.add_table(rows=2, cols=4)
    table_kpi.alignment = WD_TABLE_ALIGNMENT.CENTER
    
    headers_kpi = ["Registros Totales", "Visitas Realizadas", "Visitas Pendientes", "% Cobertura"]
...
# PESTAÑAS PRINCIPALES (CONFIGURADAS SEGÚN EL ROL DEL USUARIO)
...
with tab_dash:
    if df_base.empty:
        ...
        st.sidebar.download_button(
            ...
            data=doc_bytes,
            file_name=f"Informe_Resumen_{obtener_hora_quito().strftime('%Y%m%d')}.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            use_container_width=True
        )

        # CÁLCULO DINÁMICO DE INDICADORES POR REGISTROS REALES DE LA MATRIZ
        ...
        st.markdown('<p class="author-credit">Elaborado por Gabriela Armendariz</p>', unsafe_allow_html=True)
        st.caption(f"📌 Trabajando con la Matriz Cargar del Día: **{fecha_display}** (Total filas en esta matriz: **{tot_registros}**)")
...
# =============================================================================
# PESTAÑA 2: MÓDULO ADMINISTRADOR (SOLO VISIBLE PARA ADMINISTRADORES)
# =============================================================================
if st.session_state.get('rol_actual') == "Administrador":
    with tab_admin:
        ...
        with tab_sub_logs:
            ...
            st.caption("A continuación se muestra el registro con fecha y hora de cada usuario que ha iniciado sesión.")
