...
# Estilos CSS (Incluye ocultamiento completo de marca de agua y botones flotantes)
st.markdown("""
    <style>
    ...
    /* OCULTAR MENÚ, PIE DE PÁGINA Y BOTONES FLOTANTES DE STREAMLIT */
    footer {visibility: hidden; display: none !important;}
    #MainMenu {visibility: hidden;}
    header {visibility: hidden;}
    [data-testid="stHeader"] {display: none;}
    [data-testid="stStatusWidget"] {display: none !important;}
    .viewerBadge_container__18v {display: none !important;}
    </style>
...
# -----------------------------------------------------------------------------
# DICCIONARIO DE USUARIOS AUTORIZADOS Y ROLES
# -----------------------------------------------------------------------------
...
def obtener_ip_conexion():
    ...
    try:
        ...
        return "IP Externa / Red Privada"

# -----------------------------------------------------------------------------
# CONTROL DE SESIÓN Y LOGS DE INGRESO
# -----------------------------------------------------------------------------
...
def registrar_ingreso(usuario, rol):
    ...
    ip_cliente = obtener_ip_conexion()
    
    nuevo_log = pd.DataFrame([{
        "Usuario": usuario, 
        "Rol": rol, 
        "FechaHora": fecha_hora, 
        "IP_Conexion": ip_cliente
    }])
    
    if os.path.exists(ARCHIVO_LOGS):
        df_logs = pd.read_csv(ARCHIVO_LOGS)
        df_logs = pd.concat([df_logs, nuevo_log], ignore_index=True)
    else:
        df_logs = nuevo_log
        
    df_logs.to_csv(ARCHIVO_LOGS, index=False)
...
# -----------------------------------------------------------------------------
# FUNCIONES PARA ESTILIZAR TABLAS CON COLORES SUAVES (PASTEL)
# -----------------------------------------------------------------------------
def aplicar_estilo_tabla_agencias(df):
    def estilo_visitada(val):
        if val == "Sí":
            return 'background-color: #D1FAE5; color: #065F46; font-weight: bold;' # Verde pastel suave
        elif val == "No":
            return 'background-color: #FEE2E2; color: #991B1B; font-weight: bold;' # Rojo/Rosa pastel suave
        return ''

    styler = df.style.applymap(estilo_visitada, subset=['Visitada']) if 'Visitada' in df.columns else df.style
    
    if 'PromedioGral' in df.columns:
        styler = styler.background_gradient(cmap='Blues', subset=['PromedioGral'])
...
with tab_dash:
    if df_base.empty:
        ...
        # MOSTRAR TABLA CON COLORES PASTEL SUAVES
        df_display = df_kpi_vista[cols_mostrar].drop_duplicates()
        st.dataframe(aplicar_estilo_tabla_agencias(df_display), use_container_width=True, hide_index=True)
        ...
        with col_izq:
            ...
            st.dataframe(resumen_coord.style.background_gradient(cmap='Blues', subset=['AgenciasAsignadas', 'AgenciasVisitadas']), use_container_width=True, hide_index=True)
...
# =============================================================================
# PESTAÑA 2: MÓDULO ADMINISTRADOR Y LOGS (SOLO ADMINISTRADORES)
# =============================================================================
if st.session_state.get('rol_actual') == "Administrador":
    with tab_admin:
        ...
        with tab_sub_carga:
            ...
            with col_up1:
                ...
                fecha_subida = st.date_input("🗓️ Fecha de Carga de la Matriz:", datetime.now().date())
        ...
        with tab_sub_logs:
            st.markdown("### 📋 Historial de Ingresos de Usuarios (Con Dirección IP)")
            st.caption("Registro detallado de fecha, hora e IP de cada persona que accede.")
