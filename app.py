...
def guardar_excel_seguro(df: pd.DataFrame, filepath: str = ARCHIVO_EXCEL) -> bool:
    """Guarda en un archivo temporal y realiza un reemplazo atómico para evitar corrupción en disco."""
...
def registrar_ingreso(usuario, rol):
    """Registra inicios de sesión en modo append atómico para asegurar histórico continuo."""
...
def cargar_datos():
    ...
    if not path or os.path.getsize(path) == 0:
        ...
        # Reintentos de lectura en caso de bloqueo o reescritura temporal
        for _ in range(3):
            try:
