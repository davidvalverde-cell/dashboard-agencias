...
st.markdown("""
    ...
    /* Estilo para convertir los botones de métrica en tarjetas visuales */
    div.stButton > button {
        width: 100%;
...
# Filtrar dataset para coordinadores
...
# Aplicar filtros regionales y de zona a los datos
...
# TARJETAS DE MÉTRICA INTERACTIVAS (BOTONES DE CLIC)
k1, k2, k3, k4 = st.columns(4)

with k1:
    if st.button(f"Agencias Asignadas\n\n# {tot_asig}", key="btn_asig"):
        st.session_state['filtro_kpi'] = "Todas"

with k2:
    if st.button(f"Agencias Visitadas\n\n# {tot_vis}", key="btn_vis"):
        st.session_state['filtro_kpi'] = "Visitadas"

with k3:
    if st.button(f"Agencias No Visitadas\n\n# {tot_novis}", key="btn_novis"):
        st.session_state['filtro_kpi'] = "No Visitadas"

with k4:
    if st.button(f"% Cobertura\n\n# {pct_cob:.1f}%", key="btn_cob"):
        st.session_state['filtro_kpi'] = "Todas"

st.markdown("<br>", unsafe_allow_html=True)

# FILTRADO DINÁMICO DE LA VISTA SEGÚN EL BOTÓN KPI CLIQUEADO
...
# DESPLIEGUE DEL DETALLE AL HACER CLIC
