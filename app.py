...
# Estilos CSS (Ocultamiento de marcas de agua y botones flotantes)
st.markdown("""
    <style>
...
# -----------------------------------------------------------------------------
# FUNCIONES PARA ESTILIZAR TABLAS ROBUSTAS (COMPATIBLES CON PANDAS 1.X Y 2.X)
# -----------------------------------------------------------------------------
def aplicar_estilo_tabla_agencias(df):
    ...
    styler = df.style
    if 'Visitada' in df.columns:
        # Detectar método según versión de pandas (.map en Pandas 2.1+ / .applymap en Pandas <2.1)
        if hasattr(styler, "map"):
            styler = styler.map(estilo_visitada, subset=['Visitada'])
        else:
            styler = styler.applymap(estilo_visitada, subset=['Visitada'])
...
with tab_dash:
    if df_base.empty:
        ...
        # TABLA CON ESTILO DE COLORES SUAVES PASTEL
