...
# -----------------------------------------------------------------------------
# FUNCION ROBUSTA Y COMPATIBLE 100% PARA ESTILIZAR TABLAS CON COLORES PASTEL
# -----------------------------------------------------------------------------
def aplicar_estilo_tabla_agencias(df):
    def resaltar_columnas(col):
        if col.name == 'Visitada':
            return [
                'background-color: #D1FAE5; color: #065F46; font-weight: bold;' if v == 'Sí' 
                else ('background-color: #FEE2E2; color: #991B1B; font-weight: bold;' if v == 'No' else '') 
                for v in col
            ]
        return ['' for _ in col]

    styler = df.style.apply(resaltar_columnas)
...
with tab_dash:
    if df_base.empty:
        ...
        # TABLA CON ESTILO Y COLORES PASTEL ROBUSTOS
