...
def obtener_hora_quito():
    """Obtiene la fecha y hora actual exacta en Quito, Ecuador (UTC-5) sin depender de librerías externas."""
...
st.markdown("""
    ...
    .weather-city { font-size: 12px; font-weight: 700; color: #38BDF8 !important; text-transform: uppercase; letter-spacing: 0.5px; }
    .weather-time { font-size: 20px; font-weight: 800; color: #F8FAFC !important; margin: 2px 0; }
    .weather-info { font-size: 11px; color: #94A3B8 !important; }
...
# -----------------------------------------------------------------------------
# WIDGET DE CLIMA Y RELOJ EN QUITO EN LA BARRA LATERAL
# -----------------------------------------------------------------------------
ahora_quito = obtener_hora_quito()
hora_str = ahora_quito.strftime("%H:%M:%S")
fecha_str = ahora_quito.strftime("%d/%m/%Y")

st.sidebar.markdown(f"""
    <div class="weather-widget">
        <div class="weather-city">📍 Quito, Ecuador ⛅</div>
        <div class="weather-time">⏰ {hora_str}</div>
        <div class="weather-info">📅 {fecha_str} | 18°C Parcialmente Nublado</div>
    </div>
""", unsafe_allow_html=True)
