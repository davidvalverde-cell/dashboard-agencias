...
# -----------------------------------------------------------------------------
# LECTURA ROBUSTA DE DATOS Y CARGA HISTÓRICA
# -----------------------------------------------------------------------------
def cargar_datos():
...
# -----------------------------------------------------------------------------
# GENERACIÓN DE INFORME RESUMEN DEL DÍA EN WORD (.DOCX) - BIEN DETALLADO Y JUSTIFICADO
# -----------------------------------------------------------------------------
...
def generar_reporte_word(df_data, col_agencia, col_region, col_coord, col_fecha, cols_eval):
    ...
    # 1. RESUMEN EJECUTIVO JUSTIFICADO
    ...
    # 2. DESGLOSE POR COORDINADOR Y ZONA
    ...
    r_h3 = h3.add_run("3. Recomendaciones Ejecutivas")
    r_h3.font.name = 'Segoe UI'
    r_h3.font.color.rgb = COLOR_TITULO

    p_rec = doc.add_paragraph()
    p_rec.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p_rec.add_run("Basado en el análisis de cobertura y cumplimiento del día, se formulan las siguientes acciones:")

    recs = [
        f"Priorizar la ruta de inspección en las {tot_novis} agencias pendientes para alcanzar el 100% de cobertura en la semana.",
        "Reforzar el seguimiento en las zonas con menor índice de visitas registradas mediante alertas operativas diarias.",
        "Asegurar la carga continua y diaria de las matrices de control en la plataforma para garantizar la trazabilidad histórica."
    ]
...
# -----------------------------------------------------------------------------
tab_dash, tab_admin = st.tabs(["📊 Dashboard de Gestión", "⚙️ Carga de Matriz Histórica (Administrador)"])

# =============================================================================
# PESTAÑA 1: DASHBOARD DE CONTROL Y OPERACIONES
# =============================================================================
with tab_dash:
    if df_base.empty:
        st.warning("⚠️ No se ha encontrado ninguna matriz cargada. Por favor, ve a la pestaña '⚙️ Carga de Matriz Histórica (Administrador)' para subir el primer archivo Excel.")
    else:
        ...
        # BOTÓN EN LA BARRA LATERAL PARA DESCARGAR EL INFORME RESUMEN DEL DÍA
        st.sidebar.markdown("---")
        ...
        st.sidebar.download_button(
            label="📄 Descargar Informe Resumen del Día (Word)",
            data=doc_bytes,
            file_name=f"Informe_Resumen_Del_Dia_{datetime.now().strftime('%Y%m%d')}.docx",
        ...
        # SECCIÓN INFERIOR: DASHBOARD TRANSACCIONAL
        st.markdown("---")
        st.header("📈 Dashboard de Operaciones y Transacciones")
        st.caption("Análisis dinámico de la matriz de registros según la selección arriba.")
...
# =============================================================================
# PESTAÑA 2: MÓDULO EXCLUSIVO ADMINISTRADOR (ALMACENAMIENTO PROGRESIVO)
# =============================================================================
with tab_admin:
    ...
    if clave == "admin123":
        ...
        if archivo_nuevo and st.button("💾 Guardar y Procesar Matriz Histórica", use_container_width=True):
            try:
                ...
                # BOTÓN DIRECTO DE DESCARGA PARA SINCRONIZAR CON GITHUB EN 1 CLIC
